...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[],
	...
	std::cout << "test" << endl;
	double distance, s, t_val, base, expon;
	weights.clear();
	std::vector<LandmarkObs> car_observations;
  for (int i = 0; i < num_particles; i++) {
	  ...
	  std::cout << "test7" << endl;
		for (int j = 0; j < observations.size(); j++) {
		  ...
		  std::cout << "test8" << endl;
		  std::cout << observations.size() << endl;
		  std::cout << j << endl;
	  ...
	  std::cout << "test6" << endl;
		// Make a list of landmarks from the map to pass to dataAssociation.
		...
		for (int j = 0; j < map_landmarks.landmark_list.size(); j++) {
			...
			if (distance < sensor_range) {
				...
				map_observations[j].id = map_landmarks.landmark_list[j].id_i;
				map_observations[j].x = map_landmarks.landmark_list[j].x_f;
				map_observations[j].y = map_landmarks.landmark_list[j].y_f;
			}
		}
